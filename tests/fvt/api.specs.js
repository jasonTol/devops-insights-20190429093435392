...
(function () { //This essentially acts as the routes/apiv1.js where the fetch request goes to
    ...
    describe('Get Weather', function() { //Get Weather
      ...
      it('without city', function(done) { //Test2
        ...
        request({
      		method: 'GET',
              url: appUrl + '/api/v1/getWeather?lat=-39.05187119252786&lng=174.06645433756705'
          }, function(err, resp, body) {
          	if(err) {
          		assert.fail('Failed to get the response');
          	} else {
              ...
              assert((pbody.city === 'New Plymouth') || (pbody.city === 'new plymouth'), "Geocode does not match");
              done();
            }
        });
    	}); //End test4
        ...
        request({
      		method: 'GET',
              url: appUrl + '/api/v1/getWeather?lat=98h7&lng=98h9'
          }, function(err, resp, body) {
          	if(err) {
          		assert.fail('Failed to get the response');
          	} else {
